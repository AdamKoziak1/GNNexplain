...
def get_fraud_data(args, data_config):
    '''Loads fraud transaction data.
    
    1. The data is loaded from the csv and necessary features are mapped from fraud dataset format.
    2. The data is split into training, validation and test data.
    3. PyG Data objects are created with the respective data splits.
    '''
    
    # Allow specifying the exact transaction file path
    if hasattr(args, 'transaction_file') and args.transaction_file:
        transaction_file = args.transaction_file
    else:
        # Check if we should use the CreditCardTransactions.csv file from project root
        if args.data == 'creditcard':
            import os
            # Use repository root-relative path for credit card data
            project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            transaction_file = os.path.join(project_root, 'CreditCardTransactions.csv')
    
    logging.info(f'Loading fraud data from: {transaction_file}')
    df_edges = pd.read_csv(transaction_file)

    logging.info(f'Available Edge Features in fraud dataset: {df_edges.columns.tolist()}')

    # Map columns from fraud dataset to expected format
    df_edges['from_id'] = df_edges['CUSTOMER_ID']
    df_edges['to_id'] = df_edges['TERMINAL_ID']
    df_edges['Timestamp'] = df_edges['TX_TIME_SECONDS']
    df_edges['Is fraud'] = df_edges['TX_FRAUD']
    df_edges['Amount Received'] = df_edges['TX_AMOUNT']
    
    # Ensure timestamp starts at zero
    df_edges['Timestamp'] = df_edges['Timestamp'] - df_edges['Timestamp'].min()

    max_n_id = df_edges.loc[:, ['from_id', 'to_id']].to_numpy().max() + 1
    df_nodes = pd.DataFrame({'NodeID': np.arange(max_n_id), 'Feature': np.ones(max_n_id)})
    timestamps = torch.Tensor(df_edges['Timestamp'].to_numpy())
    y = torch.LongTensor(df_edges['Is fraud'].to_numpy())

    logging.info(f"Fraud ratio = {sum(y)} / {len(y)} = {sum(y) / len(y) * 100:.2f}%")
    logging.info(f"Number of nodes (customers & terminals) = {df_nodes.shape[0]}")
    logging.info(f"Number of transactions = {df_edges.shape[0]}")
    ...
    logging.info(f"Total train samples: {tr_inds.shape[0] / y.shape[0] * 100 :.2f}% || FR: "
            f"{y[tr_inds].float().mean() * 100 :.2f}% || Train days: {split[0][:5]}")
    logging.info(f"Total val samples: {val_inds.shape[0] / y.shape[0] * 100 :.2f}% || FR: "
        f"{y[val_inds].float().mean() * 100:.2f}% || Val days: {split[1][:5]}")
    logging.info(f"Total test samples: {te_inds.shape[0] / y.shape[0] * 100 :.2f}% || FR: "
        f"{y[te_inds].float().mean() * 100:.2f}% || Test days: {split[2][:5]}")
